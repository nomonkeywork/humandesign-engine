 ...
 * Dreamer"). It is not the standard chart filtered down; it has its own
 * structure:
 *
 ...
 *    Spleen, Root. Head, Ajna, Heart/Ego and Solar Plexus are absent;
 *  - Personality is the birth moment, exactly as in the standard chart;
 *  - Design is the moment the Moon stood 88° of arc before its birth position
 *    (about 7 days before birth) — not the Sun's 88°;
 *  - the same 13 bodies activate gates; only activations on the 15 count;
 *  - a channel needs both gates, so only six channels exist; a center is
 *    defined iff it lies on one of them.
 *
 * Verified against a MyBodyGraph "Dream Rave Overview" (see tests). What is not
 * documented in anything we have, and is therefore not asserted: how a
...
/**
 * The DreamRave Design moment: the last whole UTC minute before the Moon has
 ...
 */
export function dreamDesignMoment(birthUtcMs) {
...
/**
 * What the DreamRave shows for one view — 'personality' (birth moment),
 * 'design' (Moon −88° moment) or 'both' — from a calculateDreamRave() result.
 */
export function dreamRaveView(dreamRave, view = 'both') {
  const has = (gate, side) => dreamRave.activations.some((a) => a.gate === gate && a.side === side);
  const gates = {};
  for (const gate of new Set(dreamRave.activations.map((a) => a.gate))) {
    const p = view !== 'design' && has(gate, 'personality');
    const d = view !== 'personality' && has(gate, 'design');
    if (p || d) gates[gate] = p && d ? 'both' : p ? 'personality' : 'design';
  }
  const channels = DREAM_CHANNELS.filter((c) => c.gates.every((g) => gates[g]));
...
export function calculateDreamRave(birthDate, birthHour = 12, timezone = 0) {
  ...
  const designMs = dreamDesignMoment(birthMs);

  const birth = calculateHumanDesign(birthDate, birthHour, timezone).gates.personality;
  const p = parts(designMs);
  const iso = `${String(p.year).padStart(4, '0')}-${String(p.month).padStart(2, '0')}-${String(p.day).padStart(2, '0')}`;
  const dream = calculateHumanDesign(iso, p.hour, 0).gates.personality; // positions at that instant

  const activations = [];
  for (const body of BODIES)
    for (const [side, source] of [['personality', birth], ['design', dream]]) {
      const a = source[body];
      if (a && isDreamGate(a.gate))
        activations.push({ body, side, gate: a.gate, line: a.line, color: a.color, tone: a.tone, base: a.base, longitude: a.longitude, center: GATE_CENTER[a.gate], realm: dreamRealmOf(a.gate) });
    }

  const result = {
    designDateTime: new Date(designMs).toISOString().slice(0, 16), // UTC
    activations,
  };
  const both = dreamRaveView(result, 'both');
  return {
    ...result,
    gates: Object.keys(both.gates).map(Number).sort((a, b) => a - b),
    channels: both.channels,
    definedCenters: both.definedCenters,
    type: both.definedCenters.length ? null : 'Reflector',
    views: {
      both,
      personality: dreamRaveView(result, 'personality'),
      design: dreamRaveView(result, 'design'),
    },
  };
}
